...
interface ToolManagerModalProps {
  ...
  storageInfo?: { mode: 'database' | 'localStorage'; isDatabase: boolean };
}
    ...
    const templates = {
      text_processor: {
        ...
        return {
          content: [{
            type: "text",
            text: text ? text.toUpperCase() : "No text provided"
          }]
        };`,
        inputSchema: {
          type: 'object',
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          ...
          <div className={`border-l-4 p-4 mb-4 ${
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className={`text-sm ${storageInfo?.isDatabase ? 'text-green-700' : 'text-yellow-700'}`}>
                  <strong>Storage Mode:</strong> {storageInfo?.isDatabase ? (
                    <>🗄️ Database storage is active. Tools are stored in PostgreSQL database for persistence and multi-client access.</>
                  ) : (
                    <>📱 Local storage mode. Tools are stored in browser localStorage. For production use with persistence across clients, set USEDB=true and DATABASE_URL in your environment.</>
                  )}
                </p>
              </div>
            </div>
